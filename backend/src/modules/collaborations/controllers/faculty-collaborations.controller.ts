...
@ApiTags('Faculty - Collaborations')
@ApiBearerAuth()
@UseGuards(JwtAuthGuard, RolesGuard)
@Roles(UserRole.FACULTY)
@Controller('faculty/collaborations')
export class FacultyCollaborationsController {
  constructor(
  ...
  @Get()
  @ApiOperation({ summary: 'Browse collaborations eligible for faculty' })
  @ApiResponse({ status: 200, description: 'Faculty-eligible collaborations retrieved' })
  async getFacultyCollaborations(@Query() query: CollaborationQueryDto) {
    query.targetAudience = CollaborationAudience.FACULTY;
    return this.collaborationsService.findAll(query, true);
  }

  @Get('my-participations')
  @ApiOperation({ summary: 'List all collaboration participation requests made by authenticated faculty' })
  @ApiResponse({ status: 200, description: 'My participations retrieved' })
  async getMyParticipations(
    @CurrentUser('id') userId: string,
    @Query() query: ParticipationQueryDto,
  ) {
    return this.participationService.getMyFacultyParticipations(userId, query);
  }

  @Get('my-participations/:id')
  @ApiOperation({ summary: 'Get single participation details for authenticated faculty' })
  ...
  async getMyParticipationById(
    ...
    return this.participationService.getMyParticipationById(userId, id, UserRole.FACULTY);
  }
  ...
  async requestParticipation(
    ...
    return this.participationService.requestFacultyParticipation(userId, collaborationId, dto);
  }
  ...
  async withdrawParticipation(
    ...
    return this.participationService.withdrawParticipation(userId, id, UserRole.FACULTY);
  }
}
